...
PAGE_FILE = "c:/Users/Akram KAID/Desktop/Entreprise_transport/frontend/src/app/maintenance/page.tsx"
    ...
    <div className="space-y-6 max-w-[1600px] mx-auto p-4 md:p-6 lg:p-8 font-sans">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 opacity-0 animate-[stagger-up_0.6s_cubic-bezier(0.16,1,0.3,1)_forwards]" style={{ animationDelay: '0.1s' }}>
        <div className="glass-panel px-5 py-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group rounded-2xl">
          <div>
            <p className="text-[10px] font-accent uppercase text-white/50 tracking-widest mb-1 font-bold">Total Interventions</p>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-heading font-extrabold text-white">{totalCount}</span>
            </div>
            <p className="text-[10px] text-white/40 mt-0.5">Ordres de travail émis</p>
          </div>
          <div className="p-3 bg-white/5 rounded-full border border-white/5 group-hover:border-white/10 transition-colors">
            <Wrench className="h-5 w-5 text-white/80 group-hover:text-white transition-colors" />
          </div>
        </div>

        <div className="glass-panel px-5 py-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group rounded-2xl">
          <div>
            <p className="text-[10px] font-accent uppercase text-white/50 tracking-widest mb-1 font-bold">Travaux Terminés</p>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-heading font-extrabold text-emerald-400">{termineesCount}</span>
            </div>
            <p className="text-[10px] text-white/40 mt-0.5">Remis en circulation</p>
          </div>
        ...
        <div className="glass-panel px-5 py-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group rounded-2xl">
          <div>
            <p className="text-[10px] font-accent uppercase text-white/50 tracking-widest mb-1 font-bold">En Cours Atelier</p>
            <div className="flex items-baseline gap-2">
              <span className="text-2xl font-heading font-extrabold text-[var(--color-turbo)]">{enCoursCount}</span>
            </div>
            <p className="text-[10px] text-white/40 mt-0.5">Véhicules immobilisés</p>
          </div>
        ...
        <div className="glass-panel px-5 py-4 flex items-center justify-between hover:bg-white/[0.02] transition-colors group rounded-2xl overflow-hidden">
          <div>
            <p className="text-[10px] font-accent uppercase text-white/50 tracking-widest mb-1 font-bold">Budget Dépensé</p>
            <div className="flex items-baseline gap-2">
              <span className="text-xl font-heading font-extrabold text-[var(--color-electric-violet)] truncate">{totalCoutDZD.toLocaleString("fr-DZ")} DZD</span>
            </div>
            <p className="text-[10px] text-white/40 mt-0.5">Coût global des réparations</p>
          </div>
              ...
              {loading ? (
                <tr>
                  <td colSpan={8} className="text-center py-12">
                    ...
                    <p className="text-sm font-medium text-white/60">Chargement des ordres de travail...</p>
                  </td>
                </tr>
                ...
                <tr>
                  <td colSpan={8} className="text-center py-12">
                    ...
                    <p className="text-sm font-semibold text-white/80">Aucune intervention enregistrée</p>
                    <p className="text-xs text-white/40 mt-1">Ajustez vos filtres ou créez un nouvel ordre de travail.</p>
                  </td>
                </tr>
              ) : (
                interventions.map((i) => {
                      ...
                      <td className="py-4 px-5">
                        {i.mecanicien_nom ? (
                          <div className="flex items-center gap-2">
                            ...
                            <p className="text-xs text-white/80">{i.mecanicien_nom}</p>
                          </div>
                        ) : (
...
print("Refactored maintenance page.tsx")
