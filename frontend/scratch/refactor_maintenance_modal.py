...
MODAL_FILE = "c:/Users/Akram KAID/Desktop/Entreprise_transport/frontend/src/components/modules/maintenance/AddInterventionModal.tsx"
                    ...
                    {fields.map((field, index) => {
                      ...
                      <div key={field.id} className="flex flex-col sm:flex-row items-center gap-2 bg-[var(--color-haiti)] p-2 rounded-xl border border-white/10">
                            ...
                            render={({ field }) => (
                              <GlassSelect
                                ...
                                options={[
                                  ...
                                  ...pieces.map(p => ({ value: p.id, label: `${p.reference} - ${p.designation} (Stock: ${p.quantite_en_stock})` }))
                                ]}
                              />
                            )}
                          />
                        ...
                        {selectedPieceDetails && (
                          <div className="hidden md:block w-32 px-3 text-right">
                             <p className="text-[10px] text-white/40 font-mono">
                               ~ {(selectedPieceDetails.prix_unitaire * (watchPieces?.[index]?.quantite || 1)).toLocaleString()} DZD
                             </p>
                          </div>
                        )}
...
print("Refactored AddInterventionModal.tsx")
