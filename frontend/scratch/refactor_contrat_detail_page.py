      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 opacity-0 animate-[stagger-up_0.6s_cubic-bezier(0.16,1,0.3,1)_forwards]" style={{ animationDelay: '0.1s' }}>
        ...
        <div className="glass-panel px-5 py-4 flex flex-col justify-between hover:bg-white/[0.02] transition-colors rounded-2xl">
          ...
          <p className="text-xl font-heading font-extrabold text-white my-1 truncate">
            {new Date(contrat.date_fin_actuelle).toLocaleDateString("fr-FR")}
          </p>
            ...
            {[
              ...
              return (
                <button
                  ...
                  className={`relative flex items-center px-4 py-3 text-xs font-bold transition-colors whitespace-nowrap ${
                ...
                <div className="glass-panel p-6 rounded-2xl relative overflow-hidden group">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-y-6 gap-x-8">
                    ...
                    <div>
                      ...
                      <p className="text-sm font-bold text-white font-mono">{contrat.delai_paiement_jours}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="glass-panel p-6 rounded-2xl">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-y-6 gap-x-8">
                    <div>
                      ...
                      <p className="text-sm font-bold text-white font-mono">{contrat.taux_penalite_retard}%</p>
                    </div>
                  </div>
                </div>
              </div>
                        ...
                        <tbody className="divide-y divide-white/5">
                          {contrat.avenants.map((a) => (
                            <tr key={a.id} className="hover:bg-white/[0.02] transition-colors">
                              <td className="py-3 px-4 font-mono text-xs font-bold text-[var(--color-electric-violet)]">{a.reference}</td>
                              <td className="py-3 px-4 text-xs font-mono text-white/70">{new Date(a.date_signature).toLocaleDateString("fr-FR")}</td>
                              <td className="py-3 px-4 text-xs font-medium text-white">{a.type_modification}</td>
                              <td className="py-3 px-4 text-xs text-white/70 font-mono flex flex-col gap-0.5">
                                {a.nouveau_montant !== null && (
                                  <span>{a.nouveau_montant.toLocaleString("fr-DZ")} DZD</span>
                                )}
                                {a.nouvelle_date_fin && (
                              ...
                              <td className="py-3 px-4">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${
                                  a.statut === "ACTIF" ? "bg-emerald-400/10 text-emerald-400 border border-emerald-400/20" : "bg-white/5 text-white/50 border border-white/10"
                                }`}>
                                  {a.statut}
                                </span>
                              </td>
                        ...
                        <tbody className="divide-y divide-white/5">
                          {contrat.cautions.map((c) => (
                            <tr key={c.id} className="hover:bg-white/[0.02] transition-colors">
                              <td className="py-3 px-4 font-mono text-xs font-bold text-[var(--color-turbo)]">{c.reference}</td>
                              <td className="py-3 px-4 text-xs font-medium text-white">{c.type_caution}</td>
                              <td className="py-3 px-4 text-xs font-mono text-white">{c.montant.toLocaleString("fr-DZ")}</td>
                              <td className="py-3 px-4 text-xs text-white/70">{c.banque_emettrie || "N/A"}</td>
                              <td className="py-3 px-4 text-[11px] font-mono text-white/70">
                                {new Date(c.date_emission).toLocaleDateString("fr-FR")} <br />
                                {new Date(c.date_validite).toLocaleDateString("fr-FR")}
                              </td>
                              <td className="py-3 px-4">
                                <span className={`inline-flex items-center px-2 py-0.5 rounded-full text-[9px] font-bold uppercase ${
            ...
            {activeTab === "documents" && (
              <div className="space-y-4">
                <GlassDocumentManager 
                  entityType="contrat" 
                  entityId={contrat.id} 
                  initialDocuments={contrat.documents} 
                  onDocumentAdded={fetchDetail} 
                  onDocumentDeleted={fetchDetail}
                />
              </div>
            )}
          </div>
            ...
            <div className="relative pl-6 border-l-2 border-white/10 space-y-6">
              ...
              {contrat.avenants.map((av, idx) => (
                <div key={av.id} className="relative">
                  ...
                  <p className="text-[10px] font-accent uppercase tracking-widest text-[var(--color-electric-violet)] font-bold">Avenant {av.reference}</p>
                  <p className="text-[11px] text-white/70 mt-0.5">{av.type_modification}</p>
                  <p className="text-[11px] font-mono text-white/50">{new Date(av.date_signature).toLocaleDateString("fr-FR")}</p>
                </div>
              ))}
              ...
              <div className="relative">
                ...
                <p className="text-sm font-bold text-white mt-0.5">
                  {new Date(contrat.date_fin_actuelle).toLocaleDateString("fr-FR", { day: 'numeric', month: 'long', year: 'numeric' })}
                </p>
                <div className="mt-2">
                  {isExpired ? (
...
with open(file_path, "w", encoding="utf-8") as f:
    f.write(content)
print("ContratDetailPage rewritten successfully.")
