...
// Generic interface matching the backend Document response structure
...
interface AddInterventionDocumentModalProps {
...
export function AddInterventionDocumentModal({ interventionId, isOpen, onClose, onSuccess, defaultType }: AddInterventionDocumentModalProps) {
  ...
  useEffect(() => {
    if (defaultType) {
      ...
      reset({ type: "Facture", url_fichier: "" });
    }
  }, [defaultType, reset]);
  ...
  const onSubmit = async (data: DocumentFormValues) => {
    try {
      ...
      if (pendingFiles.length === 0) {
        setServerError("Veuillez sélectionner un fichier à uploader.");
