...
import { X, FileText, Calendar, Percent, Plus, Trash2, Building2 } from "lucide-react";
...
const devisSchema = z.object({
  ...
  date_validite: z.string().min(1, "Date requise"),
...
export function AddDevisModal({ isOpen, onClose, onSuccess }: AddDevisModalProps) {
  ...
  useEffect(() => {
    if (isOpen) {
      ...
      api.get("/partenaires").then(res => {
        // Assume API returns a list of partenaires, filter clients
  ...
  const onSubmit = async (data: DevisFormValues) => {
    ...
    try {
      ...
      setError(err.detail || err.message || "Erreur lors de la création du devis");
    } finally {
  ...
  return (
    <AnimatePresence>
      <div className="fixed inset-0 z-[100] flex items-center justify-center p-4">
        ...
        >
          {/* Top accent line */}
          <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-transparent via-[var(--color-turbo)] to-transparent opacity-50" />

          {/* Header */}
          <div className="flex items-center justify-between p-6 border-b border-white/5 shrink-0">
            <div className="flex items-center gap-3">
              <div className="p-2 rounded-xl bg-[var(--color-turbo)]/10 border border-[var(--color-turbo)]/20">
                <FileText className="w-5 h-5 text-[var(--color-turbo)]" />
              </div>
              <div>
                <h2 className="text-xl font-heading font-bold text-white">Nouveau Devis</h2>
                <p className="text-[10px] font-accent uppercase tracking-widest text-[var(--color-turbo)] mt-1">Création de Proposition Commerciale</p>
              </div>
            </div>
            <button
              onClick={onClose}
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="overflow-y-auto custom-scrollbar flex-1">
            <div className="p-6 space-y-8">
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-accent uppercase tracking-widest text-white/40 ml-1">Client Destinataire *</label>
                    <Controller
                      name="client_id"
                      control={control}
                      render={({ field }) => (
                  ...
                  <div className="space-y-1.5 md:col-span-2">
                    <label className="text-[10px] font-accent uppercase tracking-widest text-white/40 ml-1">Objet de la Prestation *</label>
                    <div className="relative">
                      <FileText className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                      <input
                        type="text"
                        {...register("objet")}
                        placeholder="Ex: Prestation de transport ponctuelle Alger - Oran"
                        className="w-full pl-10 pr-4 py-2 bg-black/20 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-[var(--color-turbo)] transition-colors text-sm"
                      />
                    </div>
                    {errors.objet && <p className="text-xs text-red-400 ml-1">{errors.objet.message}</p>}
                  </div>
                  ...
                  <div className="space-y-1.5">
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full pl-10 pr-4 py-2 bg-black/20 border border-white/10 rounded-xl text-white focus:outline-none focus:ring-1 focus:ring-[var(--color-turbo)] transition-colors text-sm [color-scheme:dark]"
                      />
                    </div>
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-accent uppercase tracking-widest text-white/40 ml-1">Date de validité *</label>
                    <div className="relative">
                      <Calendar className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-white/40" />
                      <input
                        type="date"
                    ...
                    {errors.date_validite && <p className="text-xs text-red-400 ml-1">{errors.date_validite.message}</p>}
                  </div>
                </div>
              </div>

              {/* Section 2: Lignes de Prestation */}
              <div className="space-y-4">
                <div className="flex items-center justify-between border-b border-white/5 pb-2">
                  <h3 className="text-sm font-bold text-white/80">Lignes de Prestation</h3>
                  <button
                    type="button"
                    onClick={() => append({ service: "", description: "", quantite: 1, prix_unitaire: 0 })}
                    className="inline-flex items-center gap-1 text-[10px] font-accent uppercase tracking-widest text-[var(--color-turbo)] hover:text-white transition-colors"
                  >
                    ...
                    <div key={item.id} className="grid grid-cols-12 gap-3 items-start bg-white/5 p-3 rounded-xl border border-white/10 relative group">
                      <div className="col-span-12 md:col-span-3 space-y-1">
                        ...
                        <input
                          ...
                          className="w-full px-3 py-1.5 bg-black/20 border border-white/10 rounded-lg text-white placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-[var(--color-turbo)] transition-colors text-sm"
                        />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="space-y-4">
                  ...
                  <div className="space-y-1.5">
                    <label className="text-[10px] font-accent uppercase tracking-widest text-white/40 ml-1">Conditions de règlement</label>
                    <textarea
                      {...register("conditions_reglement")}
                      placeholder="Ex: Paiement à 30 jours fin de mois..."
                      rows={3}
                      className="w-full px-4 py-2 bg-black/20 border border-white/10 rounded-xl text-white placeholder:text-white/20 focus:outline-none focus:ring-1 focus:ring-[var(--color-turbo)] transition-colors text-sm resize-none custom-scrollbar"
                    />
                  </div>
                </div>
                ...
                <div className="bg-black/20 border border-white/5 rounded-xl p-6 flex flex-col justify-center space-y-3">
                  ...
                  <div className="flex justify-between items-end">
                    <span className="text-xs font-accent uppercase tracking-widest text-[var(--color-turbo)]">Total TTC</span>
            ...
            <div className="p-6 border-t border-white/5 bg-black/20 flex justify-end gap-3 shrink-0">
              ...
              <button
                type="submit"
                disabled={isSubmitting}
                className="flex items-center gap-2 px-6 py-2 rounded-xl text-xs font-bold text-[#181230] bg-[var(--color-turbo)] border border-[#ffe133] hover:brightness-110 transition-all disabled:opacity-50"
              >
                {isSubmitting ? (
