...
export function AddEmployeeDocumentModal({
  ...
  const {
    ...
    defaultValues: {
      type: "CNI",
      url_fichier: "/assets/documents/doc_rh.pdf",
    },
  });
      ...
      <div className="w-full max-w-lg rounded-xl bg-surface border border-border shadow-xl overflow-hidden">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-border bg-table-header">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-base font-semibold text-text-primary">Ajouter un Document RH</h2>
              <p className="text-xs text-text-secondary">CNI, Extrait de naissance, Carte Chifa, Contrat</p>
            </div>
          </div>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          ...
          <div>
            <label className="block text-xs font-semibold text-text-primary mb-1">
              Chemin / URL du document
            </label>
            <input
              {...register("url_fichier")}
              placeholder="/assets/documents/cni.pdf"
