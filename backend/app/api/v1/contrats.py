...
@router.get("", response_model=ContratListResponse, summary="List Commercial Contracts")
def list_contrats(
...
@router.get("/{contrat_id}", response_model=ContratDetail, summary="Get Detailed Contract Dossier")
def get_contrat(contrat_id: UUID, db: Session = Depends(get_db)):
    ...
    # Calculate total financial amount including amendments
...
@router.post("", response_model=ContratRead, status_code=status.HTTP_201_CREATED, summary="Create New Contract")
...
@router.put("/{contrat_id}", response_model=ContratRead, summary="Update Contract")
...
@router.delete("/{contrat_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete / Archive Contract")
...
@router.post("/{contrat_id}/avenants", response_model=AvenantRead, status_code=status.HTTP_201_CREATED, summary="Add Amendment (Avenant)")
def create_contract_avenant(contrat_id: UUID, data: AvenantCreate, db: Session = Depends(get_db)):
    ...
    # Automatically extend contract date or update amount if requested
