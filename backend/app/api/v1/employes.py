...
@router.get("", response_model=EmployeListResponse, summary="List Employees (Chauffeurs & Mécaniciens)")
def list_employes(
...
@router.get("/{employe_id}", response_model=EmployeDetail, summary="Get Detailed Employee Dossier")
def get_employe(employe_id: UUID, db: Session = Depends(get_db)):
    ...
    # Fetch driver's license if chauffeur
    permis_data = None
    ...
    # Fetch interventions if mecanicien
    interventions_list = []
        ...
        interventions = (
            ...
            .filter(
                or_(
                    ...
                    Intervention.mecaniciens.any(id=employe_id),
                ),
...
@router.post("", response_model=EmployeRead, status_code=status.HTTP_201_CREATED, summary="Create Employee")
def create_employe(data: EmployeCreate, db: Session = Depends(get_db)):
    ...
    if data.type_employe == TypeEmploye.CHAUFFEUR:
        ...
        # Create attached Permis if license details provided
...
@router.put("/{employe_id}", response_model=EmployeRead, summary="Update Employee Details")
