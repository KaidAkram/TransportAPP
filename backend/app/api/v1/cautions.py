...
@router.get("", response_model=CautionListResponse, summary="List Bank Cautions")
def list_cautions(
...
@router.get("/{caution_id}", response_model=CautionDetail, summary="Get Detailed Caution Record")
...
@router.post("", response_model=CautionRead, status_code=status.HTTP_201_CREATED, summary="Create New Caution")
...
@router.put("/{caution_id}", response_model=CautionRead, summary="Update Caution")
...
@router.delete("/{caution_id}", status_code=status.HTTP_204_NO_CONTENT, summary="Delete / Archive Caution")
...
@router.post("/{caution_id}/generate-pdf", response_model=CautionRead, summary="Generate Bank Guarantee PDF Certificate")
def generate_caution_document_pdf(caution_id: UUID, db: Session = Depends(get_db)):
    ...
    # Save/Update Document in documents table
    existing_doc = (
        db.query(Document)
