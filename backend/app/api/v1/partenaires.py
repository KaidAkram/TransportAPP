...
@router.get("", response_model=PartenaireListResponse, summary="List CRM Partners (Clients & Fournisseurs)")
...
@router.get("/{partenaire_id}", response_model=PartenaireDetail, summary="Get Detailed Partner Dossier")
def get_partenaire(partenaire_id: UUID, db: Session = Depends(get_db)):
    ...
    # Fetch attached corporate documents
    docs = (
        db.query(Document)
...
@router.post("", response_model=PartenaireRead, status_code=status.HTTP_201_CREATED, summary="Create Partner")
def create_partenaire(data: PartenaireCreate, db: Session = Depends(get_db)):
    ...
    # Create nested contacts if provided
...
@router.put("/{partenaire_id}", response_model=PartenaireRead, summary="Update Partner")
...
@router.patch("/{partenaire_id}/archive", response_model=PartenaireRead, summary="Archive Partner")
...
def add_contact(partenaire_id: UUID, data: ContactCreate, db: Session = Depends(get_db)):
    ...
    # If new contact is principal, unset any existing principal
